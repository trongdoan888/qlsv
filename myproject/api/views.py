...
def create_teacher_api(request):
    try:
        ...
        return Response(
            {"error": "Mã giảng viên đã tồn tại. Vui lòng sử dụng mã khác."}, status=400
        )
...
def login(request):
    ...
    if not username or not password:
        return Response(
            {"error": "Vui lòng cung cấp tên đăng nhập và mật khẩu."}, status=400
        )
    ...
    try:
        ...
        if user_account.password == password:
            return Response(
                {"message": "Đăng nhập thành công!", "role": user_account.role},
                status=200,
            )
        else:
...
@receiver(post_save, sender=giangvien)
def create_account_for_teacher(sender, instance, created, **kwargs):
    if created:
        random_password = str(random.randint(100000, 999999))
        account.objects.create(
            id=instance.id,
...
def delete_account_for_student(sender, instance, **kwargs):
    ...
    account.objects.filter(id=instance.id).delete()


@receiver(post_delete, sender=giangvien)
def delete_account_for_teacher(sender, instance, **kwargs):
